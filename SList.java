...
public class SList {
  ...
  /** for squish use. Squish need the nth to be an SListNode not item.
   * 
   *  @param position the desired position, from 1 to length(), in the list.
   *  @return not the item but the SListNode itself
   */
  public Object nthObj(int position) {
	    SListNode currentNode;
  ...
  public void squish() {
    ...
    for ( ; startSearch < size; ) {
    	if (nth(startSearch).equals(nth(startSearch+1))) {
    		SListNode former = (SListNode)(nthObj(startSearch));		// Must use (SListNode) first.
    		SListNode latter = (SListNode)(nthObj(startSearch+1));
  ...
  public void twin() {
    ...
    // All SListNodes but the last one.
    for ( ; startSearch < size; startSearch+=2) {
    	SListNode currentNode = (SListNode)nthObj(startSearch);
    	SListNode nextNode = (SListNode)nthObj(startSearch+1);
    	
    	SListNode newNode = new SListNode(nth(startSearch), nextNode);
